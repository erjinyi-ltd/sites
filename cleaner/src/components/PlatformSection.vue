...
defineProps<{ copy: SiteCopy['platforms'] }>()
</script>
  ...
  <section id="platforms" class="platform-section">
    <div class="content-width">
      ...
      <div class="platform-grid">
        <article v-for="item in copy.items" :key="item.name" :class="`tone-${item.statusTone}`">
...
.platform-grid { display: grid; grid-template-columns: repeat(3, minmax(0, 1fr)); gap: 18px; margin-top: 42px; }
...
@media (max-width: 1000px) { .platform-grid { grid-template-columns: 1fr 1fr; } .tone-building { grid-column: 1 / -1; } }
@media (max-width: 680px) { .platform-section { padding-block: 88px; } .platform-grid { grid-template-columns: 1fr; } .tone-building { grid-column: auto; } }
</style>
